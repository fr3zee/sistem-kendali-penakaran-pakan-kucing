...
DATA=ROOT.parent.parent/'antigravity (md & csv)'/'master_dataset_160.csv'
OUT=ROOT/'reproduksi_verifikasi'
...
def main():
 ...
 outputs={k:save(k,v) for k,v in frames.items()}; missing=[str(p) for p in [DATA,*[ROOT/x for x in BASE],ROOT/'rekomendasi_uji_tahap3.csv'] if not p.exists()]; bad=[] if missing else sum((compare(ROOT/k,v) for k,v in outputs.items()),[])
 hashes={p.name:digest(p) for p in [DATA,*[ROOT/x for x in BASE]] if p.exists()}; entries=[manifest(DATA,'master_dataset')]+[manifest(ROOT/x,'baseline') for x in BASE]+[manifest(p,'reproduction') for p in outputs.values()]
