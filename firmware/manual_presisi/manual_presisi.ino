/**
 * SKENARIO 2 — OPEN LOOP PRESISI (Tanpa Kontrol)
 * =================================================
 * Penelitian: Perancangan dan Implementasi Kontrol PID Adaptif
 *             Gain Scheduling pada Sistem Pakan Kucing Otomatis
 *
 * PRINSIP:
 *   - Servo dibuka ke sudut MINIMUM (20°) — aliran sangat lambat
 *   - Tidak ada algoritma kontrol (PID/GS) — hanya threshold
 *   - Servo ditutup saat massa >= (setpoint - EARLY_STOP_MARGIN)
 *   - Fokus: presisi takaran, bukan kecepatan
 *
 * HIPOTESIS:
 *   - Settling time SANGAT LAMBAT
 *   - MAE% RENDAH (hasil mendekati target karena aliran lambat)
 *   - Standar deviasi RENDAH (konsisten karena aliran terkendali)
 *
 * PERBEDAAN DARI SKENARIO 1:
 *   S1: Sudut 40° (penuh) → cepat tapi tidak presisi
 *   S2: Sudut 20° (min)   → lambat tapi presisi
 *
 * METRIK YANG DIREKAM:
 ...
 *   - SUDUT_MIN = 20° (sama dengan GS v3 dan Fixed PID v7)
...
const int SUDUT_MIN   = 20;   // Sudut minimum — open loop presisi
const int SUDUT_BUKA  = 40;   // Untuk bridging hammer saja

// ============================================================
// PARAMETER SISTEM
// ============================================================
const int           INTERVAL_SAMPLING = 100;    // ms — 10 Hz
const unsigned long TIMEOUT_MS        = 120000; // ms — batas observasi/safety disamakan

// Anti-bridging — SAMA dengan skenario lain [5][6]
// Catatan: pada S2 bridging lebih sering karena sudut kecil
// Anti-bridging tetap aktif agar kondisi perbandingan adil
...
void loop() {
  ...
  // Timeout safety — lebih panjang karena aliran lambat
  ...
  if (sekarang - tSampling >= INTERVAL_SAMPLING) {
    ...
    // Early stop — threshold sederhana, bukan kontrol [5][6]
    ...
    // Setelah bridging pulse selesai, kembali ke SUDUT_MIN (bukan SUDUT_BUKA)
    ...
    if (bridgingPulseAktif) {
      ...
      if (elapsed < 300) {
        ...
        myServo.write(SUDUT_BUKA);        // hammer: buka penuh sesaat
      } else {
        bridgingPulseAktif = false;
        myServo.write(SUDUT_MIN);         // S2: kembali ke sudut MINIMUM
      }
    }

    // Log data — format sama dengan skenario lain
    ...
    Serial.print("%,OPEN_SLOW,"); Serial.print(SUDUT_MIN);
...
// Setelah shake, kembali ke SUDUT_MIN
// ============================================================
void prosesShake(unsigned long sekarang) {
  if (!shakeAktif &&
    ...
    if (!bridgingPulseAktif)
      myServo.write(constrain(SUDUT_MIN + SHAKE_AMPLITUDO, SUDUT_MIN, SUDUT_BUKA));
  } else if (shakeAktif &&
             sekarang - tShakeMulai >= (unsigned long)SHAKE_DURASI_MS) {
    shakeAktif = false;
    if (!bridgingPulseAktif) myServo.write(SUDUT_MIN);  // kembali ke sudut minimum
  } else if (shakeAktif &&
             sekarang - tShakeMulai >= (unsigned long)(SHAKE_DURASI_MS / 2)) {
    if (!bridgingPulseAktif) myServo.write(SUDUT_MIN);
  }
}
...
void mulaiDispensing() {
  ...
  // Buka servo ke sudut MINIMUM — open loop presisi
  myServo.write(SUDUT_MIN);

  Serial.println("=== TRIAL START ===");
  Serial.print("Controller: "); Serial.println("Manual Presisi");
  ...
  Serial.print("Servo_deg: "); Serial.println(SUDUT_MIN);
...
void cetakParameter() {
  ...
  Serial.println("[SKENARIO 2 - OPEN LOOP PRESISI]");
  Serial.print("Setpoint   : "); Serial.print(setpoint, 1); Serial.println("g");
  Serial.print("Sudut      : "); Serial.print(SUDUT_MIN); Serial.println("deg (tetap, open loop presisi)");
  ...
  Serial.print("TIMEOUT    : "); Serial.print(TIMEOUT_MS / 1000); Serial.println("s (diperpanjang)");
  Serial.println("Mode       : Tidak ada kontrol — servo minimum dari awal");
  Serial.println("=================================");
}
...
void tampilHeader() {
  ...
  Serial.println("=========================================");
  Serial.println("Mode: Servo tetap 20deg, tidak ada PID");
  Serial.println("Tujuan: Benchmark presisi tanpa kontrol");
  Serial.println("Hipotesis: Presisi tapi settling time lama");
  Serial.println("Timeout: 120 detik (diperpanjang)");
  Serial.println("-----------------------------------------");
